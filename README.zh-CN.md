...
| 进程管理繁琐 | 自动管理网关生命周期 |
| 多 AI 供应商切换 | 拆分后的“模型服务商”与“模型配置”设置页 |
...
自定义频道账号 ID 现在会强制使用 OpenClaw 兼容的规范格式：仅支持小写字母、数字、连字符和下划线，必须以字母或数字开头，且最长 64 个字符。

### ⏰ 定时任务自动化
调度 AI 任务自动执行。定义触发器、设置时间间隔，让 AI 智能体 7×24 小时不间断工作。
现在定时任务表单支持选择外部投递通道，并且在多账号通道下显式选择发送账号，同时继续复用已知会话目标作为投递建议。
调度编辑器现在提供 `每隔`、`固定时间` 和 `Cron` 三种模式，列表卡片里的每周任务也会显示星期名称，便于快速查看。
...
### 🔐 安全的供应商集成
连接多个 AI 供应商（OpenAI、Anthropic 等），凭证安全存储在系统原生密钥链中。GeeClaw 现在把 provider 接入和模型分配拆开：先在 **设置 → 模型服务商** 中配置账号和可用模型，再到 **设置 → 模型配置** 中选择默认聊天模型以及图像 / 视频等模型槽位。OpenAI 同时支持 API Key 与浏览器 OAuth（Codex 订阅）登录。内置的 **GeeClaw** provider 只需要填写 API key，OpenClaw 运行时会通过本地 loopback 透明代理访问上游，因此配置里只会留下 `http://127.0.0.1:<port>/proxy` 和环境变量形式的 key 引用，而不会写入真实上游地址。
...
首次启动 GeeClaw 时，应用会先检查是否需要登录；确认完成后，**设置向导** 会继续完成这些步骤：

1. **欢迎页** – 选择语言，并确认应用已准备好进入配置
2. **环境检查** – 校验内置 OpenClaw 运行时和相关服务
3. **开始设置** – 安装首次启动需要的默认本地工具
4. **进入主界面** – 准备完成后直接进入应用

模型相关配置现在统一放到主界面内完成：

- 打开 **设置 → 模型服务商** 添加 provider 账号及其可用模型目录。
- 内置的 **GeeClaw** provider 是固定配置：只允许输入 API key，真实上游 Base URL 会被 GeeClaw 自己的本地透明代理隐藏起来。
...
如果改动涉及首次启动、设置文案或其他用户可见行为，请在同一个 PR 中同步更新 `README.md` 和 `README.zh-CN.md`。
