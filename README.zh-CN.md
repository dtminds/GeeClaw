...
当 Agent 正在流式回复时，输入框仍可继续编辑：最多一条后续消息默认本地排队，也可以把这条已排队消息提升为当前 run 的引导指令。
...
- 在启动 Gateway 之前，GeeClaw 会先检查 `~/.openclaw-geeclaw/extensions/lossless-claw/package.json`；只有当 `lossless-claw` 缺失或版本低于 GeeClaw 要求时，才会重新安装或升级。
...
- Gateway readiness 以进程/传输状态和 OpenClaw 的 `ready`、`system-presence`、`health`、`status` 等核心信号为准；memory、Dreams 或频道失败会显示为能力降级，而不是全局 Gateway 故障。
- 本地诊断信息通过 Host API 的 `GET /api/gateway/diagnostics` 暴露，包含 Gateway 状态、heartbeat/RPC 诊断和能力快照，供排障工具消费。
- 可用以下命令确认监听进程：
