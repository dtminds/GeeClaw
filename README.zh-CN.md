...
### 🔐 安全的供应商集成
连接多个 AI 供应商（OpenAI、Anthropic 等），凭证安全存储在系统原生密钥链中。GeeClaw 现在把 provider 接入和模型分配拆开：先在 **设置 → 模型服务商** 中配置账号和可用模型，再到 **设置 → 模型配置** 中选择默认聊天模型以及图像 / 视频等模型槽位。OpenAI 同时支持 API Key 与浏览器 OAuth（Codex 订阅）登录。
