...
打开 **设置 → 快捷操作** 可以配置面向划词文本的悬浮快捷动作。

- 每个快捷动作都可以绑定独立的全局快捷键和默认模式。
- 悬浮窗口会出现在鼠标附近，窗口内可以在翻译、回复、查询和自定义提示词之间切换，无需重新按热键。
- GeeClaw 会优先尝试读取原生选中文本，必要时可回退到剪贴板文本。
- 在 macOS 上，直接读取选中文本需要开启辅助功能权限；在 Windows 上，是否可读取取决于前台应用对 UI Automation 的支持情况。
- 当前的“粘贴结果”仍是基于剪贴板的尽力而为方案，暂不保证在所有应用里都能做到真实模拟粘贴或原位替换。
