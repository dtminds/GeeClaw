...
对于 release CI，GeeClaw 现在也可以直接消费 GitHub Releases 上预先构建好的 OpenClaw sidecar，而不是在每个主包 release job 里重复打同一份 runtime。精确 pin 的产物定义放在 [`runtime-artifacts/openclaw-sidecar/version.json`](runtime-artifacts/openclaw-sidecar/version.json)。在 sidecar draft release 产出并把生成的 `openclaw-sidecar-version.json` 回填到这个追踪文件之前，请保持 `enabled: false`。
