...
- GeeClaw 内置 Provider 会包含一个 `auto` 模型。本地透明代理会每 30 分钟刷新一次手动上传到 `https://b0.dtminds.com/geeclaw/geeclaw-provider-config.json` 的 Provider 配置，用来选择真实上游 URL、限制允许请求的模型列表，并从 `autoModels` 中选择当前应用版本已经在 provider registry 里合法定义的第一个模型作为 `auto`。远程配置请求会自动追加时间戳参数以穿透 CDN 缓存。这个代理配置不会发布新的 UI 模型，也不会覆盖 OpenClaw runtime 配置；新增可见模型仍然需要更新应用内 provider registry。
...
- 设置 `GEECLAW_PROVIDER_CONFIG_URL` 可以在开发模式覆盖 GeeClaw Provider 代理配置 URL。远程配置请求会自动追加 `ts` 查询参数。
