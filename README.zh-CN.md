...
pnpm run openclaw-runtime:prepare  # 确保本地开发所需的独立 runtime 已就绪
...
对于 release CI，GeeClaw 现在也可以直接消费 GitHub Releases 上预先构建好的 OpenClaw sidecar，而不是在每个主包 release job 里重复打同一份 runtime。精确 pin 的产物定义放在 [`runtime-artifacts/openclaw-sidecar/version.json`](runtime-artifacts/openclaw-sidecar/version.json)。当前支持发布的 sidecar 目标是 `darwin-arm64`、`darwin-x64` 和 `win32-x64`；Windows on Arm 机器统一使用 GeeClaw 的 x64 安装包，并沿用同一条 x64 自动更新链路。在 sidecar draft release 产出并把生成的 `openclaw-sidecar-version.json` 回填到这个追踪文件之前，请保持 `enabled: false`。
