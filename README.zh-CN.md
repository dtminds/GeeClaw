...
在智能体广场中，即使某个 preset 只支持部分平台，它也会继续展示。GeeClaw 会在卡片上直接显示支持平台标签，支持通过 **查看详情** 查看该 preset 的 Agent ID、workspace、预置技能、受管文件和平台可用性，并在当前系统不受支持时禁用添加按钮。
...
- 如果 preset 锁定了 persona 文件，后端快照会按文件粒度控制可编辑性。当前受管控策略会锁定 `IDENTITY.md`，同时允许继续编辑 `USER.md`、`MEMORY.md` 和 `SOUL.md`，Persona 抽屉也会显示后端返回的受管控提示。
