...
import { handleRuntimeEventState, handleRuntimeToolEvent } from './runtime-event-handlers';

export function createRuntimeEventActions(set: ChatSet, get: ChatGet): Pick<RuntimeActions, 'handleChatEvent' | 'handleAgentEvent'> {
  return {
    ...
    handleAgentEvent: (event: Record<string, unknown>) => {
      handleRuntimeToolEvent(set, get, event);
    },
  };
}
