...
type EvolutionProposalCardData = {
  proposalId: string;
  signature?: string;
  description: string;
...
function isEvolutionProposalToolName(name: string): boolean {
...
function extractEvolutionProposalCardData(
  name: string,
  input: unknown,
  result: unknown,
...
function EvolutionProposalCard({
  ...
  const handleDecision = useCallback(async (decision: 'approve' | 'reject') => {
    ...
    try {
      const command = decision === 'approve' ? '批准' : '拒绝';
  ...
  const handleOpenDraft = useCallback(async () => {
    ...
    await openLocalPath(proposal.draftPath);
  }, [proposal.draftPath]);
...
function ToolCard({
  ...
  if (evolutionProposalCard) {
    return <EvolutionProposalCard proposal={evolutionProposalCard} status={status} />;
  }
