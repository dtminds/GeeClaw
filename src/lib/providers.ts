...
export function normalizeProviderModelEntries(
  ...
  for (const value of models ?? []) {
    ...
    results.push({
      ...
      ...(normalizeProviderModelModalities(value?.input) ? { input: normalizeProviderModelModalities(value?.input) } : {}),
      ...(normalizeOptionalPositiveInteger(value?.contextWindow) ? { contextWindow: normalizeOptionalPositiveInteger(value?.contextWindow) } : {}),
      ...(normalizeOptionalPositiveInteger(value?.maxTokens) ? { maxTokens: normalizeOptionalPositiveInteger(value?.maxTokens) } : {}),
    });
  }
