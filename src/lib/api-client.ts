...
const GATEWAY_WS_DIAG_FLAG = 'geeclaw:gateway-ws-diagnostic';
...
async function resolveGatewayPort(): Promise<number> {
  ...
  const port = typeof status?.port === 'number' && status.port > 0 ? status.port : 28788;
...
function shouldLogApiRequests(): boolean {
  try {
    return import.meta.env.DEV || window.localStorage.getItem('geeclaw:api-log') === '1';
  } catch {
