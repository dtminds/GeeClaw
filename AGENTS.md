...
GeeClaw is a cross-platform **Electron desktop app** (React 19 + Vite + TypeScript) providing a GUI for the OpenClaw AI agent runtime. It uses pnpm as its package manager (pinned version in `package.json`'s `packageManager` field).
...
| Task | Command |
|------|---------|
| Install deps + download uv + Node/npm | `pnpm run init` |
...
- **`pnpm run init`**: This is a convenience script that runs `pnpm install`, `pnpm run uv:download`, and `pnpm run node:download`. Either run `pnpm run init` or run the steps separately.
- **Gateway startup**: When running `pnpm dev`, the OpenClaw Gateway process starts automatically on port 28788. It takes ~10-30 seconds to become ready. Gateway readiness is not required for UI development—the app functions without it (shows "connecting" state).
...
- **Avoid Claude-style warm panels**: Do not introduce the old warm beige modal surfaces (`#f3f1e9`, `#eeece3`, `#1a1a19`, `#151514`) for dialogs, sheets, or modal form sections.
- **Use shared modal surface classes**: Prefer the reusable classes in `src/styles/globals.css`:
  - `modal-card-surface` for the outer modal card
  - `modal-section-surface` for grouped settings/help sections inside a modal
  - `modal-field-surface` for inputs, textareas, segmented controls, and inline field containers
- **Use shared modal typography/actions**:
  - `modal-title` for modal titles
  - `modal-description` for modal subtitle/description text
  - `modal-close-button` for the top-right dismiss button
  - `modal-footer` for the bottom action row
