...
describe('prepareWinSpawn', () => {
  ...
  it('quotes args that contain spaces on Windows with shell', () => {
    ...
    const result = prepareWinSpawn(
      'node',
      ['C:\\Users\\John Doe\\script.js', '--port', '28788'],
      true,
    );
    expect(result.shell).toBe(true);
    expect(result.command).toBe('node');
    expect(result.args).toEqual(['"C:\\Users\\John Doe\\script.js"', '--port', '28788']);
  });
...
describe('normalizeNodeRequirePathForNodeOptions', () => {
  ...
  it('converts backslashes to forward slashes on Windows', () => {
    setPlatform('win32');
    expect(normalizeNodeRequirePathForNodeOptions('C:\\Users\\70954\\AppData\\Roaming\\geeclaw\\gateway-fetch-preload.cjs'))
      .toBe('C:/Users/70954/AppData/Roaming/geeclaw/gateway-fetch-preload.cjs');
  });
