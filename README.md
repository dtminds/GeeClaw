...
Each agent also exposes an **Active Evolution** toggle in **Agent Settings → General**. Turning it off removes the `hermes-evolution` skill from that agent's explicit skill membership and adds `evolution_proposal` to the agent-local `tools.deny` list.
