...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[geeclaw-validate] ${provider} HTTP ${status}`);
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (modelsResult.status === 404) {
    console.log(
      `[geeclaw-validate] ${providerType} /models returned 404, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  const baseRoot = rawBase.replace(/\/messages$/i, '');
  const base = baseRoot.endsWith('/v1') ? baseRoot : `${baseRoot}/v1`;
  ...
  if (
    ...
    console.log(
      `[geeclaw-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );
    const messagesUrl = base.endsWith('/messages') ? base : `${base}/messages`;
