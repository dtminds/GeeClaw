...
function capabilityFromPayload(payload: unknown, durationMs?: number): GatewayCapabilityProbe {
  return {
    state: 'healthy',
    checkedAt: Date.now(),
    durationMs,
...
function capabilityFromError(error: unknown, durationMs?: number): GatewayCapabilityProbe {
  return {
    state: 'degraded',
    checkedAt: Date.now(),
    durationMs,
...
export class GatewayCapabilityMonitor {
  ...
  recordOpenClawHealth(payload: unknown, durationMs?: number): void {
    this.openclawHealth = capabilityFromPayload(payload, durationMs);
  }

  recordOpenClawStatus(payload: unknown, durationMs?: number): void {
    this.openclawStatus = capabilityFromPayload(payload, durationMs);
  }
  ...
  recordCapabilitySuccess(name: GatewayCapabilityName, payload: unknown, durationMs?: number): void {
    this.setCapability(name, capabilityFromPayload(payload, durationMs));
  }

  recordCapabilityFailure(name: GatewayCapabilityName, error: unknown, durationMs?: number): void {
    this.setCapability(name, capabilityFromError(error, durationMs));
  }
