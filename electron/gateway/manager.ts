...
import { launchGatewayProcess, type ManagedGatewayProcess } from './process-launcher';
...
export class GatewayManager extends EventEmitter {
  ...
  private async initDeviceIdentity(): Promise<void> {
    ...
    try {
      const identityPath = path.join(app.getPath('userData'), 'geeclaw-device-identity.json');
  ...
  /**
   * Attach to an already-running Gateway without spawning a new process.
   * Used when GeeClaw starts after a system-managed Gateway is already up.
   */
  async attachToExistingGatewayIfAvailable(options?: {
    ...
    const existing = await findExistingGatewayProcess({
      ...
      terminateForeignProcess: options?.terminateForeignProcess ?? false,
    ...
    try {
      ...
      logger.info(`Attached to existing Gateway on port ${existing.port}`);
      return true;
    } catch (error) {
      logger.warn('Failed to attach to existing Gateway:', error);
      return false;
    }
  }
  ...
  async stop(options?: { shutdownExternal?: boolean }): Promise<void> {
    ...
    if (shutdownExternal) {
      ...
      if (remainingListeners.length > 0) {
        logger.warn(
          `Gateway stop incomplete: port ${stopPort} is still occupied by PIDs ${remainingListeners.join(', ')}`,
        );
        clearPendingGatewayRequests(this.pendingRequests, new Error('Gateway stop incomplete'));
        this.restartController.resetDeferredRestart();
        this.setStatus({
          state: 'error',
          error: `Gateway listener still running on port ${stopPort}`,
        });
        throw new Error(`Gateway listener still running on port ${stopPort}`);
      }
  ...
  async restart(): Promise<void> {
    if (this.restartController.isRestartDeferred({
      ...
      logger.info(
        `Gateway restart deferred (state=${this.status.state}, startLock=${this.startLock})`,
      );
    ...
    if (this.restartInFlight) {
      logger.info('Gateway restart already in progress, joining existing request');
      await this.restartInFlight;
      return;
    }

    logger.info(`Gateway restart requested (state=${this.status.state}, ownsProcess=${this.ownsProcess})`);
    ...
    try {
      await this.restartInFlight;
      logger.info(`Gateway restart completed (state=${this.status.state}, ownsProcess=${this.ownsProcess})`);
    } finally {
