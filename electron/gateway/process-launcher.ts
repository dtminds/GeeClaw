...
export type ManagedGatewayProcess = ChildProcess | Electron.UtilityProcess;

const GATEWAY_FETCH_PRELOAD_SOURCE = `'use strict';
(function () {
  var _f = globalThis.fetch;
  if (typeof _f !== 'function') return;
  if (globalThis.__geeclawFetchPatched) return;
  globalThis.__geeclawFetchPatched = true;

  globalThis.fetch = function geeclawFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://www.geeclaw.cn';
      flat['X-Title'] = 'GeeClaw';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__geeclawPatched) {
        cp.__geeclawPatched = true;
