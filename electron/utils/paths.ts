 ...
 *
 * Uses a GeeClaw-specific profile state directory (~/.openclaw-geeclaw) so the
 * embedded runtime does not collide with a system-wide OpenClaw installation.
 */
export function getOpenClawConfigDir(): string {
  return join(homedir(), '.openclaw-geeclaw');
}
...
/**
 * Get GeeClaw config directory
 */
export function getGeeClawConfigDir(): string {
  return join(homedir(), '.geeclaw');
}

/**
 * Get GeeClaw logs directory
 */
 ...
 * - Development: prefer openclaw-runtime/node_modules/openclaw, then fall back to node_modules/openclaw
 */
export function getOpenClawDir(): string {
  ...
  const runtimeDir = join(process.cwd(), 'openclaw-runtime', 'node_modules', 'openclaw');
