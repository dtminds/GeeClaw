...
export class AppUpdater extends EventEmitter {
  ...
  private autoUpdater: AutoUpdaterType | null = null;
  ...
  constructor() {
    ...
    try {
      ...
      this.autoUpdater = updaterModule.autoUpdater;
    } catch (error) {
      this.loadError = error instanceof Error ? error : new Error(String(error));
      this.status = {
        status: 'error',
        error: `Auto-updater unavailable: ${this.loadError.message}`,
      };
      logger.error('[Updater] Failed to load electron-updater:', this.loadError);
      return;
    }
  ...
  private getAutoUpdaterOrThrow(): AutoUpdaterType {
    if (!this.autoUpdater) {
      throw new Error(this.loadError?.message || 'electron-updater is unavailable');
    }
  ...
  quitAndInstall(): void {
    ...
    setQuitting();
    this.getAutoUpdaterOrThrow().quitAndInstall();
  }
  ...
  setAutoDownload(enable: boolean): void {
    this.getAutoUpdaterOrThrow().autoDownload = enable;
  }
