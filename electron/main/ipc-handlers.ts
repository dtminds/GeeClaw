...
function registerUnifiedRequestHandlers(gatewayManager: GatewayManager): void {
  ...
  const handleSafetySettingsChange = async () => {
    const settings = await getAllSettings();
        ...
        case 'provider': {
          ...
          if (request.action === 'setApiKey') {
            ...
            try {
              ...
              const providerType = provider?.type || providerId;
              try {
                await syncProviderApiKeyToRuntime(providerType, providerId, apiKey, gatewayManager);
              } catch (err) {
            ...
            try {
              ...
              try {
                ...
                if (previousKey) {
                  ...
                  await syncProviderApiKeyToRuntime(existing.type, providerId, previousKey, gatewayManager);
                } else {
                  await providerService.deleteLegacyProviderApiKey(providerId);
 ...
 * Direct read/write to ~/.openclaw-geeclaw/openclaw.json (bypasses Gateway RPC)
 */
...
function registerOpenClawHandlers(gatewayManager: GatewayManager): void {
  const scheduleGatewayChannelRestart = (reason: string): void => {
    refreshGatewayAfterConfigChange(gatewayManager, reason);
  };
  ...
  ipcMain.handle('openclaw:isReady', async () => {
    const status = await getConfiguredOpenClawRuntime();
    return status.packageExists;
  });

  // Get the resolved OpenClaw directory path (for diagnostics)
  ipcMain.handle('openclaw:getDir', async () => {
    const status = await getConfiguredOpenClawRuntime();
    return status.dir;
  });

  // Get the OpenClaw config directory (~/.openclaw-geeclaw)
  ipcMain.handle('openclaw:getConfigDir', () => {
    return getOpenClawConfigDir();
  });

  // Get the OpenClaw skills directory (~/.openclaw-geeclaw/skills)
  ...
  ipcMain.handle('channel:getConfig', async (_, channelType: string, accountId?: string) => {
    try {
      const config = await getChannelConfig(channelType, accountId);
...
/**
 * WeCom Login Event Forwarding
 */
function registerWeComHandlers(mainWindow: BrowserWindow): void {
  weComLoginManager.on('qr', (data) => {
    if (!mainWindow.isDestroyed()) {
      mainWindow.webContents.send('channel:wecom-qr', data);
    }
  });

  weComLoginManager.on('success', (data) => {
    if (!mainWindow.isDestroyed()) {
      logger.info('wecom:login-success', { accountId: data?.accountId });
      mainWindow.webContents.send('channel:wecom-success', data);
    }
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[geeclaw-validate] validating provider type: ${providerType}`);
...
function registerShellHandlers(): void {
  const normalizeShellPath = (targetPath: string): string => {
 ...
 * Stage files to ~/.openclaw-geeclaw/media/outbound/ for gateway access
 */
 ...
 * The JSONL file lives at: ~/.openclaw-geeclaw/agents/<agentId>/sessions/<suffix>.jsonl
...
function registerSessionHandlers(): void {
  ipcMain.handle('session:delete', async (_, sessionKey: string) => {
    try {
      ...
      if (!result.success) {
        logger.warn(`[session:delete] Failed for ${sessionKey}: ${result.error}`);
      }
      return result;
    } catch (err) {
